...
export default function* mergeSortBottomUp(items, arrMax){
    ...
    while(merged.length !== 1){
        ...
        while (n < merged.length){
            if(n + 1 !== merged.length){
                ...
                merged = [...merged.slice(0, n), combined, ...merged.slice(n + 2)]
    ...
    for(let i = 0; i < merged[0].length; i++){
        merged[0][i].active = true
        yield newElements(merged.flat(), arrMax) 
        merged[0][i].active = false
    }
    

    yield newElements(merged.flat().map((el) => ({val: el.val, active: true})), arrMax)

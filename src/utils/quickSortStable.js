...
export default function* quickSortStable(items, arrMax){
    ...
    function sortQ(ar){
        if (ar.length <= 1){
            ...
            changes.push(JSON.parse(JSON.stringify([...smaller, pivot, ...greater])))
            return JSON.parse(JSON.stringify([...sortQ(smaller), pivot, ...sortQ(greater)]))
    ...
    for (let one of changes){
        ...
        baseCopy = [...baseCopy.slice(0, indexes[0]), ...one, ...baseCopy.slice(indexes[indexes.length - 1] + 1)]

...
export default function* bucketSort(items, arrMax){
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        yield newElements([...JSON.parse(JSON.stringify(buckets)).flat(), ...JSON.parse(JSON.stringify(arr)).slice(i + 1)], arrMax)
    }
    ...
    for (let k = 0; k < n; k++) {
        for (let i = 1; i < buckets[k].length; i++){
            ...
            yield newElements(JSON.parse(JSON.stringify(buckets)).flat(), arrMax)
            if(j > 0) buckets[k][j].active = false
            if(i > 0) buckets[k][i].active = false
    
            buckets[k] = [...buckets[k].slice(0,j), x, ...buckets[k].slice(j,i), ...buckets[k].slice(i + 1)]

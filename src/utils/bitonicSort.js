...
export default function* bitonicSort(items, arrMax){
    ...
    function compAndSwap(a, i, j, dir) {
        if ((a[i].val > a[j].val && dir === 1) || (a[i].val < a[j].val && dir === 0)){
    ...
    for (let el of changes){
        ...
        if(!el.slice(0, base.length).every((x, idx) => x.val === prev[idx].val)){
            yield newElements(el.slice(0, base.length), arrMax)
        }  
        prev = el
    }
    
    yield newElements(w.map((el) => ({val: el.val, active: true})).slice(0, base.length), arrMax)

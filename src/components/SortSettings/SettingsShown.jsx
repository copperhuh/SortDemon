...
import { doChangeSize, doChangeSortType, doChangeSpeed, doShuffle, doChangeSortingOngoing } from "../../redux/Actions";
import { connect } from "react-redux";

const SettingsShown = ({toggleShow, shuffle, size, speed, sortType, changeSize, changeSpeed, changeSortType, changeSortingOngoing}) => {
    const MAX = 500
    ...
    const handleChange = (e) =>  {
        ...
        if(Number.isInteger(val) && val >= 0 && val <= MAX){
            setSettings((prevSettings) => ({ ...prevSettings, [e.target.name]: val }))
            ...
            <div className="container">
                ...
                <div className="size">
                    ...
                    <Slider
                        ...
                        aria-label="size"
                    />
                    <input type="text" name='size' value={settings.size} onChange={handleChange} className="num-field"/>
                </div>
                <div className="speed">
                    SPEED
                    <Slider
                        ...
                        aria-label="speed"
                    />
                    <input type="text" name='speed' value={settings.speed} onChange={handleChange} className="num-field"/>
                </div>
                <div className="shuffle-type-buttons">
                    ...
                    <div className="shuffle-container">
                        <button 
                            className={`shuffle-btn ${settings.sortType === 'random' ? 'active' : null}`} 
                            onClick={() => changeSortTypeState('random')}>
                                RANDOM
                        
                        </button>
                        <button 
                            className={`shuffle-btn ${settings.sortType === 'nearly-sorted' ? 'active' : null}`} 
                    ...
                    <div className="shuffle-container">
                        <button 
                            className={`shuffle-btn ${settings.sortType === 'few-unique' ? 'active' : null}`} 
                            onClick={() => changeSortTypeState('few-unique')}>
                                FEW UNIQUE
                        
                        </button>
                        <button 
                            className={`shuffle-btn ${settings.sortType === 'reverse' ? 'active' : null}`} 
